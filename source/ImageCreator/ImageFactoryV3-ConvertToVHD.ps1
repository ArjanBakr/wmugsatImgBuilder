...
foreach($wim in $wims){
    ...
    if($UEFI -eq $True){
        ...
        if((Test-Path -Path $DestinationFile) -ne $true){
            ...
            E:\ImageCreator\Convert-VIAWIM2VHD.ps1 -SourceFile $wim.FullName -DestinationFile $DestinationFile -Disklayout UEFI -SizeInMB 80000 -Index 1
        }else{
    ...
    if($BIOS -eq $True){
        ...
        if((Test-Path -Path $DestinationFile) -ne $true){
            Write-Host "About to create $DestinationFile"
            E:\ImageCreator\Convert-VIAWIM2VHD.ps1 -SourceFile $wim.FullName -DestinationFile $DestinationFile -Disklayout BIOS -SizeInMB 80000 -Index 1
        }else{
            Write-Host "$DestinationFile already exists"
        }
    }
